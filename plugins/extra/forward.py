...
async def forward(client, message): 
    msg = await message.reply_text(message.chat.id, "**Forward any message from the Target channel\nBot should be admin at both the Channels**") 
    t_chat = msg.forward_from_chat.id 
    msg1 = await message.reply_text(message.chat.id, "**Send Starting Message From Where you want to Start forwarding**") 
    msg2 = await message.reply_text(message.chat.id, "**Send Ending Message from same chat**") 
